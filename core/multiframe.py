...
TIMEFRAMES = {
    "15m": {"binance": "15m", "weight": 0.15},
    "1h":  {"binance": "1h",  "weight": 0.20},
...
HIGHER_TFS = ["4h", "1d"]
LOWER_TFS = ["15m", "1h"]
LOOKBACK = {"15m": 200, "1h": 200, "4h": 150, "1d": 100}
...
def analyze_symbol_multiframe(symbol):
    ...
    return _consolidate_signals(symbol, tf_results)


def _consolidate_signals(symbol, tf_results):
    if not tf_results:
        return None
    ...
    if bias and entry and bias == entry:
        ...
        confidence = min(combined * 1.3, 0.95)
    ...
    if entry and not bias:
        confidence = (sum(all_buy) if entry == "BUY" else sum(all_sell)) * 0.8
        return {
            "action": entry,
